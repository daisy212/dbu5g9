...
void SetSysClock(void) {
  ...
  //
  // Set power range for target frequency of 160Mhz
  //
  RCC->PLL1CFGR = ((RCC->PLL1CFGR & ~RCC_PLL1CFGR_PLL1SRC) | RCC_PLL1CFGR_PLL1SRC_1);    // Select HSI as PLL1 clock source
  RCC->AHB3ENR |= (RCC_AHB3ENR_PWREN);                                                   // Enable PWR register
  PWR->VOSR    |= (3u << PWR_VOSR_VOS_Pos)                                               // Set VOS to Range1
               |  (PWR_VOSR_BOOSTEN);                                                    // Enable EPOD booster to allow systemclock above 55Mhz
  while ((PWR->VOSR & PWR_VOSR_VOSRDY) == 0) {
    ;  // Wait for voltage level to be ready
  }
  ...
  //
  FLASH->ACR = ((FLASH->ACR & FLASH_ACR_LATENCY) | FLASH_ACR_LATENCY_4WS);    // Set latency to 4 wait states
  //
  // Configure PLL1
  //
  RCC->PLL1CFGR = (3u << RCC_PLL1CFGR_PLL1SRC_Pos)    // Select HSE as input source
                | (3u << RCC_PLL1CFGR_PLL1RGE_Pos);   // Set input frequency range to 8-16Mhz
  RCC->PLL1DIVR = (9u << RCC_PLL1DIVR_PLL1N_Pos)      // Set PLL1 multiplier to 10
                | (1u << RCC_PLL1DIVR_PLL1P_Pos)      // Set PLL1P to 2         (-> pll1_p_ck = vco1_ck / 2)
                | (1u << RCC_PLL1DIVR_PLL1Q_Pos)      // Set PLL1Q to 2         (-> pll1_q_ck = vco1_ck / 2)
                | (0u << RCC_PLL1DIVR_PLL1R_Pos);     // Bypass PLL1R divider   (-> pll1_r_ck = vco1_ck)
  RCC->CR      |= RCC_CR_PLL1ON;                      // Power on PLL1
  while ((RCC->CR & RCC_CR_PLL1RDY) == 0) {
    ;  // Wait PLL1 to be stable
  }
  RCC->PLL1CFGR = (RCC_PLL1CFGR_PLL1REN);    // Enable PLL1R output
  //
  // Switch system clock to PLL1 output R
  //
  RCC->CFGR1 |= (3u << RCC_CFGR1_SW_Pos);
  while ((RCC->CFGR1 & RCC_CFGR1_SWS) == 0) {
    ;  // Wait system clock to be switched
  }
}
