...
*/
  ...
  #define FS_NOR_HW_NOR_CLK_HZ          15000000uL      // Frequency of the clock supplied to NOR flash device
#endif
  ...
  #define FS_NOR_HW_USE_MEM_MODE        0               // Enables or disables the reading of NOR flash data via system memory.
...
**********************************************************************
essai with octospi1 
pa3 clk, pa6 io3, pa7 io2, pb0 io2, pb1 io0, pe11 ncs
...
*
*       Octo-SPI registers
#define OCTOSPI_BASE_ADDR       0x420D2400uL    // octospi2
*/
#define OCTOSPI_BASE_ADDR       0x420D1400uL    // octospi1
...
#define AHB2ENR1_GPIOAEN        0
#define AHB2ENR1_GPIOBEN        1
#define AHB2ENR1_GPIOCEN        2
#define AHB2ENR1_GPIODEN        3
#define AHB2ENR1_GPIOEEN        4
#define AHB2ENR1_GPIOFEN        5
#define AHB2ENR1_GPIOGEN        6
#define AHB2ENR1_GPIOHEN        7
#define AHB2ENR1_GPIOIEN        8

//#define AHB2ENR1_OCTOSPIMEN     21     // octospi2
#define AHB2ENR1_OCTOSPIMEN     20     // octospi1

//#define AHB2ENR2_OCTOSPI2EN     8      // octospi2
#define AHB2ENR2_OCTOSPI1EN     4      // octospi1

#define CCIPR2_OCTOSPISEL_BIT   20     // common o1 et o2
...
#define NOR_DQS_BIT             12    // Port F
#define NOR_CLK_BIT             4     // Port F
#define NOR_D0_BIT              0     // Port F
#define NOR_D1_BIT              1     // Port F
#define NOR_D2_BIT              2     // Port F
#define NOR_D3_BIT              3     // Port F
...
// pa3 clk, pa6 io3, pa7 io2, pb0 io2, pb1 io0, pe11 ncs
#define NOR_CLK_BIT             4     // Port F
#define NOR_D3_BIT              6     // Port A
#define NOR_D2_BIT              7     // Port A
#define NOR_D1_BIT              0     // Port B
#define NOR_D0_BIT              1     // Port B
#define NOR_CS_BIT              11     // Port E
*/

// pa3 clk, pa6 io3, pa7 io2, pb0 io2, pb1 io0, pe11 ncs
#define NOR_CLK_BIT             3     // Port A
#define NOR_D3_BIT              0     // Port C
#define NOR_D2_BIT              3     // Port C
#define NOR_D1_BIT              2     // Port C
#define NOR_D0_BIT              1     // Port C
#define NOR_CS_BIT              2     // Port A
...
#define DCR1_CSHT_DLYBYP           3
#define DCR1_CSHT_BIT           8
#define DCR1_DEVSIZE_BIT        16
#define DCR1_DEVSIZE_MAX        0x16u
...
*    Calculates the clock divider of Octio-SPI controller.
*
*  Parameters
...
static void _Reset(void) {
  RCC_AHB2RSTR2 |=  (1uL << AHB2ENR2_OCTOSPI1EN);
  RCC_AHB2RSTR2 &= ~(1uL << AHB2ENR2_OCTOSPI1EN);
  OCTOSPI_CR   = 0
               | (1uL << CR_EN_BIT)                           // Enable the Octo-SPI unit.
               | (0uL << CR_TCEN_BIT)                         // Enable the timeout for the memory mapped read operations.
               | (0x7uL << CR_FTHRES_BIT)
  ...
  OCTOSPI_DCR1 = 0
               ...
               | (0uL                << DCR1_CSHT_BIT)        // The CS signal stays 3 clock cycles high between commands. This setting was taken from an ST sample.
               | (DCR1_MTYP_MICRON << DCR1_MTYP_BIT)
               | (1uL                 << DCR1_CKMODE_BIT)
               |(1uL<<DCR1_CSHT_DLYBYP)
               ;
//#define DCR1_MTYP_MICRON        0uL
//#define DCR1_MTYP_MACRONIX      1uL
//#define DCR1_MTYP_STANDARD      2uL
...
static int _HW_Init(U8 Unit) {
  ...
  //
  RCC_AHB2ENR1 |= 0
               | (1uL << AHB2ENR1_GPIOAEN)
               | (1uL << AHB2ENR1_GPIOBEN)
               | (1uL << AHB2ENR1_GPIOCEN)
               | (1uL << AHB2ENR1_GPIOEEN)
               | (1uL << AHB2ENR1_GPIOFEN)
               | (1uL << AHB2ENR1_GPIOHEN)
               | (1uL << AHB2ENR1_GPIOIEN)
               | (1uL << AHB2ENR1_OCTOSPIMEN)
               ;
  RCC_AHB1ENR  |= 1uL << AHB1ENR_DCACHE1EN;         // Acc. to [1] it is necessary to enable this clock even when the data cache is not used.
  RCC_AHB2ENR2 |= 1uL << AHB2ENR2_OCTOSPI1EN;
  ...
  //
  for (;;) {
    if ((RCC_AHB2RSTR2 & (1uL << AHB2ENR2_OCTOSPI1EN)) == 0) {
      break;
    }
  }
// Alternate function 0xa  : ospi1, port1, v package

  //
  // CS is an output signal and is controlled by the OCTOSPI unit.
  //
  GPIOA_MODER   &= ~(MODER_MASK   << ( NOR_CS_BIT       << 1));
  GPIOA_MODER   |=  (MODER_ALT    << ( NOR_CS_BIT       << 1));
  // sup à 8 ???
  GPIOA_AFRL    &= ~(AFR_MASK     << ((NOR_CS_BIT - 0)  << 2));
  GPIOA_AFRL    |=  (0xauL        << ((NOR_CS_BIT - 0)  << 2));
  GPIOA_OSPEEDR &= ~(OSPEEDR_MASK << ( NOR_CS_BIT       << 1));
  GPIOA_OSPEEDR |=  (OSPEEDR_HIGH << ( NOR_CS_BIT       << 1));
  //
  // DQS is an output signal and is controlled by the OCTOSPI unit.
  /*
  GPIOF_MODER   &= ~(MODER_MASK   << ( NOR_DQS_BIT      << 1));
  GPIOF_MODER   |=  (MODER_ALT    << ( NOR_DQS_BIT      << 1));
  GPIOF_AFRH    &= ~(AFR_MASK     << ((NOR_DQS_BIT - 8) << 2));
  GPIOF_AFRH    |=  (0x5uL        << ((NOR_DQS_BIT - 8) << 2));
  GPIOF_OSPEEDR &= ~(OSPEEDR_MASK << ( NOR_DQS_BIT      << 1));
  GPIOF_OSPEEDR |=  (OSPEEDR_HIGH << ( NOR_DQS_BIT      << 1));
*/
  //
  // CLK is an output signal controlled by the OCTOSPI unit.
  //
  GPIOA_MODER   &= ~(MODER_MASK   << ( NOR_CLK_BIT      << 1));
  GPIOA_MODER   |=  (MODER_ALT    << ( NOR_CLK_BIT      << 1));
  GPIOA_AFRL    &= ~(AFR_MASK     << ((NOR_CLK_BIT - 0) << 2));
  GPIOA_AFRL    |=  (0xauL        << ((NOR_CLK_BIT - 0) << 2));
  GPIOA_OSPEEDR &= ~(OSPEEDR_MASK << ( NOR_CLK_BIT      << 1));
  GPIOA_OSPEEDR |=  (OSPEEDR_HIGH << ( NOR_CLK_BIT      << 1));
  //
  // D0 is an input/output signal controlled by the OCTOSPI unit.
  //
  GPIOC_MODER   &= ~(MODER_MASK   << ( NOR_D0_BIT      << 1));
  GPIOC_MODER   |=  (MODER_ALT    << ( NOR_D0_BIT      << 1));
  GPIOC_AFRL    &= ~(AFR_MASK     << ((NOR_D0_BIT - 0) << 2));
  GPIOC_AFRL    |=  (0xauL        << ((NOR_D0_BIT - 0) << 2));
  GPIOC_OSPEEDR &= ~(OSPEEDR_MASK << ( NOR_D0_BIT      << 1));
  GPIOC_OSPEEDR |=  (OSPEEDR_HIGH << ( NOR_D0_BIT      << 1));
  //
  // D1 is an input/output signal controlled by the OCTOSPI unit.
  //
  GPIOC_MODER   &= ~(MODER_MASK   << ( NOR_D1_BIT      << 1));
  GPIOC_MODER   |=  (MODER_ALT    << ( NOR_D1_BIT      << 1));
  GPIOC_AFRL    &= ~(AFR_MASK     << ((NOR_D1_BIT - 0) << 2));
  GPIOC_AFRL    |=  (0xauL        << ((NOR_D1_BIT - 0) << 2));
  GPIOC_OSPEEDR &= ~(OSPEEDR_MASK << ( NOR_D1_BIT      << 1));
  GPIOC_OSPEEDR |=  (OSPEEDR_HIGH << ( NOR_D1_BIT      << 1));
  //
  // D2 is an input/output signal and is controlled by the OCTOSPI unit.
  //
  GPIOC_MODER   &= ~(MODER_MASK   << ( NOR_D2_BIT      << 1));
  GPIOC_MODER   |=  (MODER_ALT    << ( NOR_D2_BIT      << 1));
  GPIOC_AFRL    &= ~(AFR_MASK     << ((NOR_D2_BIT - 0) << 2));
  GPIOC_AFRL    |=  (0xauL        << ((NOR_D2_BIT - 0) << 2));
  GPIOC_OSPEEDR &= ~(OSPEEDR_MASK << ( NOR_D2_BIT      << 1));
  GPIOC_OSPEEDR |=  (OSPEEDR_HIGH << ( NOR_D2_BIT      << 1));
  //
  // D3 is an output signal and is controlled by the OCTOSPI unit.
  //
  GPIOC_MODER   &= ~(MODER_MASK   << ( NOR_D3_BIT      << 1));
  GPIOC_MODER   |=  (MODER_ALT    << ( NOR_D3_BIT      << 1));
  GPIOC_AFRL    &= ~(AFR_MASK     << ((NOR_D3_BIT - 0) << 2));
  GPIOC_AFRL    |=  (0x3uL        << ((NOR_D3_BIT - 0) << 2));
  GPIOC_OSPEEDR &= ~(OSPEEDR_MASK << ( NOR_D3_BIT      << 1));
  GPIOC_OSPEEDR |=  (OSPEEDR_HIGH << ( NOR_D3_BIT      << 1));
  //
...
/* if (hospi->Instance == (OCTOSPI_TypeDef *)OCTOSPI1)   instance = 0U;
        ...
        MODIFY_REG(OCTOSPIM->PCR[0],
                   (OCTOSPIM_PCR_IOHEN | OCTOSPIM_PCR_IOHSRC),
                   (OCTOSPIM_PCR_IOHEN | (0 << (OCTOSPIM_PCR_IOHSRC_Pos + 1U))));

*/

/******* Re-enable  OctoSPI after configure OctoSPI IO Manager ********/
...
**********************************************************************
*/
const FS_NOR_HW_TYPE_SPIFI FS_NOR_HW_SPIFI_STM32U5G9 = {
  _HW_Init,
  _HW_Unmap,
#if FS_NOR_HW_USE_MEM_MODE
  _HW_Map,
