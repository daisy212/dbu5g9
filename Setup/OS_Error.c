...
//lint -save -e9059 MISRA 2012 Rule 3.1, required
//lint -estring(9070,OS_Error) MISRA 2012 Rule 17.2, required
...
*    OS_ERR_TASK_STACK (120), OS_ERR_SYSTEM_STACK_OVERFLOW (125) and
*    OS_ERR_INTERRUPT_STACK_OVERFLOW (126) may occur.
...
void OS_Error(OS_STATUS ErrCode) {
  //
  // With embOS-MPU this might be called from an unprivileged task or before OS_MPU_Init().
  // Therefore, we might not be able to write to embOS variables and to call MPU API routines.
  //
#if (OS_SUPPORT_MPU == 0)
  //
