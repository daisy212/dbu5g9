...
static INLINE U16 SEGGER_RdU16LE(const U8 aData[2]) {
  ...
  return SEGGER_BSWAP_U16(*(const U16*)(aData))
#elif SEGGER_UTIL_CONFIG_BYTE_ORDER_U16 < 0
  return *(const U16*)(aData);
#else
...
static INLINE U32 SEGGER_RdU32LE(const U8 aData[4]) {
  ...
  return SEGGER_BSWAP_U32(*(volatile const U32 *)aData);
#elif SEGGER_UTIL_CONFIG_BYTE_ORDER_U32 < 0
  return *(volatile const U32*)(aData);
#else
  return  (U32)aData[0] +
         ((U32)aData[1] <<  8) +
         ((U32)aData[2] << 16) +
...
static INLINE U64 SEGGER_RdU64LE(const U8 aData[8]) {
  ...
  return *(volatile const U64*)(aData);
#else
...
*
*  Return value
*    Distance between two pointers in bytes, used e.g. as NumBytes in a buffer.
*
*  Additional information
*    Does not work in cases where pointers wrap back to 0x0 as there is
*    way to automatically detect this case.
*
*  Notes
*    (1) Distance is always U32, although 64bit pointers might be
*        farther away.
*
*    (2) Deprecated, use SEGGER_PTR_DISTANCE if you know which pointer
*        is larger in value which is basically always the case.
*/
static INLINE U32 SEGGER_GetPtrDistance(const void* p0, const void* p1) {
  U32 Distance;

  if (p0 > p1) {
    Distance = (U32)SEGGER_PTR_DISTANCE(p0, p1);
  } else {
    Distance = (U32)SEGGER_PTR_DISTANCE(p1, p0);
  }
  return Distance;
}
#endif
