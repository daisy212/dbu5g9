...
#define BL_MIN 1
void bl_menu(void)
// BackLight setting for ips display
{
   char buff[50];
   int16_t inc_p = 100;
   int16_t inc_m = 100;
   coord x = 5;
   coord y = 5;
   font_p font = LibMonoFont12x20;
   size  h = font->height();
   int16_t bl_lev= Settings.BLlevel();

   lcd_clear_buf();    // ok
   Screen.text(x, y, utf8("Backligh setting"), font, pattern(0, 255, 0));
    y += 3 * h / 2;
   int x2= Screen.text(x, y, utf8("    (+) (-) "), font, pattern(0, 255, 0));

//   y += 3 * h / 2;
    Screen.text(x, y + 3 * h / 2, utf8("    Press EXIT key to continue..."), font, pattern(0, 255, 0));
lcd_forced_refresh();
        ...
        while (true)
    {
         ...
         if ( key == KEY_ADD )
         {
            if (bl_lev + inc_p <= BL_MAX) bl_lev = bl_lev + inc_p;
            Disp_Set_BL_Level( bl_lev);  
         }
            
         if ( key == KEY_SUB ){
            if (bl_lev - inc_m > BL_MIN) bl_lev = bl_lev - inc_m;
            Disp_Set_BL_Level( bl_lev);  
        
         }
         snprintf(buff, sizeof(buff), "Backlight %d    ", bl_lev/10 );
         Screen.fill(x2+4, y, x2+300, y+h);
         Screen.text( x2+5, y, utf8(buff), font, pattern(0, 255, 0));
...
// Select a city for timezone
{
   ...
   font_p font = LibMonoFont12x20;
   size  h = font->height();
   int16_t tz_tmp = Settings.TMzone();

   lcd_clear_buf();    // ok
   Screen.text(x, y, utf8("Time Zone setting"), font, pattern(0, 255, 0));
    y += 3 * h / 2;
   int x2= Screen.text(x, y, utf8("    (+) (-) "), font, pattern(0, 255, 0));
   int x3 = LCD_WIDTH - 1;
//   y += 3 * h / 2;
...
lcd_forced_refresh();
        ...
        while (true)
    {
         ...
         if (IS_EXIT_KEY(key) || is_menu_auto_off())
         {
            Settings.TMzone(tz_tmp);
            break;
         }
         if (( key == KEY_ADD ) && ( cities[tz_tmp+1].region != NULL)) tz_tmp = tz_tmp + 1;          
         if (( key == KEY_SUB ) && ( tz_tmp > 0)) tz_tmp = tz_tmp - 1;        
         snprintf(buff, sizeof(buff), "Time Zone %s, City  %s    ", cities[tz_tmp].region, cities[tz_tmp].city );
         Screen.fill(x2+4, y, x3, y+h);
         Screen.text( x2+5, y, utf8(buff), font, pattern(0, 255, 0));
         DisplayUpdate(&hlcd, true, true);
   }
}
...
// ----------------------------------------------------------------------------
{
   ...
   Screen.text(x, y, utf8("This project is powered by SEGGER Tools"), font, pattern(0, 255, 0));
   y += h;
   Screen.text(x, y, utf8("under the Segger's Friendly License, FSL"), font, pattern(0, 255, 0));
    y += h;
   Screen.text(x, y, utf8("with grateful thanks to SEGGER team"), font, pattern(0, 255, 0));
...
#endif
    y += h +10;
   x2 = Screen.text( 5, y, utf8(buff), font, pattern(0, 255, 0));
   ...
   y += h ;
   x2 = Screen.text(5, y, utf8("IP over USB, SNTP, FTP server : usb.local (Admin,Secret) "), font, pattern(0, 255, 0));

    y += h +5;
    for (uint i = 0; i < 2; i++)
        x2 = Screen.text(5+i, y, utf8(HARD_NAME ""), font, pattern(0, 255, 0));
   snprintf(buff, sizeof(buff), "v%s, %3dMhz, %3dKb by d212", HARD_VERSION, SystemCoreClock/1000000, DB_MEM_SIZE);
//    y += h+5;
   x2 = Screen.text( x2, y, utf8(buff), font,  pattern(0, 255, 0));
   ...
   y = LCD_H / 2 + 30;
    for (uint i = 0; i < 2; i++)
        x2 = Screen.text(x+i, y, utf8(PROGRAM_NAME " "), font, pattern(0, 255, 0));
    Screen.text(x2, y, utf8("v" PROGRAM_VERSION " © 2024 C. de Dinechin"), font, pattern(0, 255, 0));
    y += h;
    Screen.text(x, y, utf8("A modern implementation of RPL, and"), font, pattern(0, 255, 0));
    y += h;
    Screen.text(x, y, utf8("a tribute to Bill Hewlett and Dave Packard"), font, pattern(0, 255, 0));

    y += 3 * h / 2;
    Screen.text(x, y, utf8("    Press EXIT key to continue..."), font, pattern(0, 255, 0));
//    lcd_refresh();
   lcd_forced_refresh();
    wait_for_key_press();
}
...
// ----------------------------------------------------------------------------
{
    ...
    MI_SET_TZmenu,                  // Standard set date menu
...
// ----------------------------------------------------------------------------
{
    ...
    switch (menu_id)
    {
    case MI_DB48_BLmenu: bl_menu(); break;
    case MI_SET_TZmenu: tz_menu(); break;
...
// ----------------------------------------------------------------------------
{
    ...
    switch (menu_id)
    {
    case MI_DB48_BLmenu:                ln = "Backlight >";             break;
    ...
    case MI_SET_TZmenu:                 ln = "Time Zone";               break;
